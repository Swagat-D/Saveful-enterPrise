...
export function AuditWorkspace() {
  ...
  const allEntries = useMemo(() => listAudit(), [version]);
  const options = useMemo(() => auditFilterOptions(allEntries), [allEntries]);
  const rows = useMemo(() => filterAudit(filters, allEntries), [filters, allEntries]);
  ...
  const setFilters = (next: AuditFilters) => {
    router.replace(`${pathname}${auditFiltersToQuery({ ...next, page: next.page || 1 })}`, { scroll: false });
  };
  const update = (patch: Partial<AuditFilters>) => setFilters({ ...filters, ...patch, page: patch.page ?? 1 });
        ...
        <section className="overflow-hidden rounded-2xl border border-black/[0.05] bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
          <header className="flex flex-col gap-3 border-b border-gray-100 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div className="min-w-0">
              <h1 className="font-saveful-bold text-xl leading-none text-gray-900 sm:text-2xl">Audit Log</h1>
              <p className="mt-1.5 font-saveful text-xs text-gray-500">
                View a record of administrative changes across your Enterprise.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="space-y-4 p-4 sm:p-5">
              ...
              <div className="space-y-3 p-3.5">
                    ...
                    >
                      <div className="grid grid-cols-1 gap-3">
                        ...
                        <FilterSelect
                          ...
                          options={[{ id: "all", name: "All" }, ...options.actions.map((action) => ({ id: action, name: action }))]}
                        />
                        <FilterSelect
                          label="User"
                          value={filters.user}
                          onChange={(next) => update({ user: next })}
                          options={[{ id: "all", name: "All" }, ...options.users.map((name) => ({ id: name, name }))]}
                        />
                        <FilterSelect
                          label="Area"
                          value={filters.area}
                          onChange={(area) => update({ area })}
                          options={[{ id: "all", name: "All" }, ...AUDIT_AREAS.map((item) => ({ id: item.id, name: item.label }))]}
                        />
                      </div>
                    </MoreFilters>
                  </div>
                ...
                <div className="hidden grid-cols-2 gap-2 lg:grid xl:grid-cols-4">
                  ...
                  <FilterSelect
                    value={filters.action}
                    onChange={(action) => update({ action })}
                    options={[{ id: "all", name: "Action: All" }, ...options.actions.map((action) => ({ id: action, name: `Action: ${action}` }))]}
                  />
                  <FilterSelect
                    value={filters.user}
                    onChange={(next) => update({ user: next })}
                    options={[{ id: "all", name: "User: All" }, ...options.users.map((name) => ({ id: name, name: `User: ${name}` }))]}
                  />
                  <FilterSelect
                    value={filters.area}
                    onChange={(area) => update({ area })}
                ...
                <table className="min-w-full text-left">
                  <thead>
                    <tr className="border-b border-gray-100 font-saveful text-[11px] uppercase tracking-wide text-gray-400">
                      ...
                      <th className="pb-2 pr-3 font-saveful">Area</th>
                      <th className="pb-2 pr-3 font-saveful">Details</th>
                  ...
                  <tbody>
                    {paged.map((entry) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 pr-3 font-saveful text-sm text-gray-600">{areaLabel(entry.area)}</td>
                        <td className="max-w-xs py-2.5 pr-3 font-saveful text-sm text-gray-600">
                          <span className="line-clamp-2">{entry.detail || entry.entity}</span>
                        </td>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 font-saveful text-xs text-gray-500">
                        {entry.actor} · {areaLabel(entry.area)}
            ...
            <p className="flex items-start gap-2 font-saveful text-xs text-gray-500">
              ...
              <span>
                Audit Log shows administrative and governance changes only. Routine operational activity is recorded in{" "}
                <Link href="/activity" className="font-saveful-semibold text-saveful-green hover:underline">
                  Activity
                </Link>
                . Records are retained for {AUDIT_RETENTION_MONTHS} months and cannot be edited or deleted.
              </span>
            </p>
          </div>
        </section>
...
function ChangeDetails({ entry, onClose }: { entry: AuditEntry; onClose: () => void }) {
  useEffect(() => {
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <DetailRow label="Changed by">
            <span className="flex items-center gap-2.5">
              ...
              <span className="font-saveful-semibold text-sm text-gray-900">{entry.actor}</span>
            </span>
          </DetailRow>
          ...
          <DetailRow label="Area">{areaLabel(entry.area)}</DetailRow>
          {entry.entity ? <DetailRow label={entityLabel(entry.area)}>{entry.entity}</DetailRow> : null}
          {entry.detail ? (
...
function actionIcon(action: string) {
  ...
  if (value.includes("reassign")) return ArrowLeftRight;
  if (value.includes("deactivat")) return Ban;
  if (value.includes("reactivat")) return RotateCcw;
  if (value.includes("user added") || value.includes("invit")) return UserPlus;
  if (value.includes("role") || value.includes("scope")) return Shield;
  if (value.includes("user")) return Users;
  if (value.includes("notification")) return Bell;
  if (value.includes("organisation") || value.includes("settings")) return Settings;
  if (value.includes("group") || value.includes("territory") || value.includes("cluster")) return Network;
  if (value.includes("added") || value.includes("add ")) return Plus;
  return Building2;
}

function entityLabel(area: AuditEntry["area"]) {
  if (area === "users") return "User";
  if (area === "structure") return "Structure";
  if (area === "settings") return "Setting";
  return "Site";
}
