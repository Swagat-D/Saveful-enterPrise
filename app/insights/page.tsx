...
export default function InsightsPage() {
  ...
  const [mode, setMode] = useState<ImpactFilterMode>("all_time");
  const [startDate, setStartDate] = useState(presetRange(30).startDate);
  const [endDate, setEndDate] = useState(presetRange(30).endDate);
  ...
  const stats = useMemo(() => getImpactStats(siteId, factor), [siteId, factor]);
  const foods = useMemo(() => getTopFoods(factor), [factor]);
  const recipients = useMemo(() => getRecipients(factor), [factor]);
  const chartData = useMemo(() => getChartSeries(range, metric, siteId), [range, metric, siteId]);
  const periodLabel = filterLabel(mode, startDate, endDate);
      ...
      actions={
        <>
          ...
          <div className="relative">
            <Button className="w-full sm:w-auto" onClick={() => setReportOpen((open) => !open)}>
    ...
    >
      ...
      <section>
        <h2 className="mb-3 font-saveful-bold text-lg text-gray-900">
          {mode === "all_time" ? "All-time impact" : `Impact · ${periodLabel}`}
        </h2>
        ...
        <div className="mt-3 flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <div>
            ...
            <p className="font-saveful text-xs text-gray-500">
              Collection rating · {stats.ratingCount} reviews
            </p>
          </div>
        </div>
      </section>
          ...
          {recipients.map((row) => {
            ...
            return (
              <article key={row.rank} className="overflow-hidden rounded-2xl border border-gray-100 bg-[#FCFCFA]">
                <button
                  type="button"
                  onClick={() => setExpandedRecipient(open ? null : row.rank)}
                  className="flex w-full items-start gap-3 p-4 text-left"
                >
                  <div
                    className={cn(
                      "flex h-10 w-10 shrink-0 items-center justify-center rounded-full",
                      row.kind === "animals" ? "bg-orange-50" : "bg-saveful-green/10",
                    )}
                  >
                    <Image
                      src={row.kind === "animals" ? "/listing/cow_front.png" : "/listing/charity_green.png"}
                      alt=""
                      width={22}
                      height={22}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-100">
                      <div
                        className={cn(
                          "h-full rounded-full",
                          row.kind === "animals" ? "bg-saveful-orange" : "bg-saveful-green",
                        )}
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className={cn("flex items-center gap-2 px-4 py-2.5", tone === "green" ? "bg-[#F0F8F3]" : "bg-[#FFF6EC]")}>
        ...
        <p className={cn("font-saveful-semibold text-sm", tone === "green" ? "text-saveful-green" : "text-saveful-orange")}>
          {title}
        </p>
      </div>
      <div className="p-4">
        <p className={cn("font-saveful-bold text-2xl tabular-nums", tone === "green" ? "text-saveful-green" : "text-saveful-orange")}>
          {value}
        </p>
